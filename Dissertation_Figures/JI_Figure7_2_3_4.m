...
[dvec5,BS5,trans5,p5] = HGLET_GHWT_BestBasis_minrelerror(dmatrixH,dmatrixHrw,dmatrixHsym,dmatrixG,GP,G);
...
figure;
semilogy(x,r2,'Color',[0 0.5 0],'LineWidth',1); row=row+1; legcell{row} = 'Laplacian Eigenvectors (L)';
hold on
semilogy(x,r1,'-c','LineWidth',1); row=row+1; legcell{row} = sprintf('HGLET (L) BB (tau=%.1f)',p1);
semilogy(x,r4,'-k','LineWidth',2); row=row+1; legcell{row} = 'Haar';
semilogy(x,r3,'-g','LineWidth',1); row=row+1; legcell{row} = sprintf('GHWT BB (tau=%.1f)',p3);
semilogy(x,r5,':m','LineWidth',1); row=row+1; legcell{row} = sprintf('Hybrid BB (tau=%.1f)',p5);
semilogy(x,r6,'Color',[0.5 0.5 0.5],'LineWidth',1); row=row+1; legcell{row} = 'Walsh';
...
[~,jmax] = size(dmatrixH);
jmax = jmax-1;
G = EditPlotSpecs(G,sprintf('sortnodes size5 CLim[0,%d] verbatim{{axis equal; axis off;}}',jmax));

BasisVisual(G,GP,BS3,dvec3);


%% Figure 7.4
BasisVisual(G,GP,BS1,dvec1);